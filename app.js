...
const { div, h2, label, input, button, form, p, small, span } = h.proxy
...
const alertBoxContent = span(' ... ')
const alertBox = h('.alert.alert-success.fade', { role: 'alert' })([
  alertBoxContent,
  h('button.close', {
    onclick: ev => {
      ev.preventDefault()
...
const bsClasses = [ 'success', 'info', 'warning', 'danger' ]
const alertClasses = bsClasses.map(el => `alert-${el}`)
let _sayTimeout
const say = (content, type) => {
  if (!content) return alertBox.classList.add('fade')
  if (type) {
    alertBox.classList.remove(...alertClasses)
    alertBox.classList.add(`alert-${type}`)
  }
  h.empty(alertBoxContent)
  h.appendChild(alertBoxContent, content)
  alertBox.classList.remove('fade')
  clearTimeout(_sayTimeout)
  _sayTimeout = setTimeout(say, 2500)
}
...
weso.processingEnd(({ data: videoHash }) => {
...
weso.processingError(({ data }) =>
  say(`Erreur lors de l'encodage : ${data}`, 'error'))

weso.uploadError(({ data }) =>
  say(`Erreur lors de l'envoi : ${data}`, 'error'))
...
weso.uploadStatus(({ data: progress }) => {
  ...
  say(`Fichier déja existant`, 'warning')
...
weso.uploadReady(({ data: { progress } }) => {
  ...
  say('Encodage de la vidéo', 'info')
  weso.publish.uploadDone()
})

const container = h('.container')
const content = h('.row.marketing', { style: { marginBottom: '220px' } })
...
const submitForm = ev => {
  ...
  uploadButton.textContent = 'Suspendre'
...
body.appendChild(container([
  ...
  content(form({ onsubmit: submitForm, style: { margin: '0 auto' } }, [
    alertBox,
    userInput.firstName,
    userInput.lastName,
    userInput.file,
